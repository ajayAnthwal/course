...
router.use(protect);
router.post("/", restrictTo("admin"), upload.single("image"), validate(createCourseSchema), courseController.createCourse);
router.patch("/:id", restrictTo("admin"), upload.single("image"), validate(updateCourseSchema), courseController.updateCourse);
